...
<div align="justify">
					...
					<ul>
						...
						<li>A the first execution, the application will show that no job exists in the job perisstence store, with a message similar to this:<br/>
							<div align="left" style="background-color: #D0D0D0"/>
								<tt>no job found in persistence store, creating a new job with 10 tasks</tt>
							</div>
						</li>
						<li>You will then see, in the node's shell console, messages like this:<br/>
							<div align="left" style="background-color: #D0D0D0"/>
								<tt>task 1 completed sucessfully</tt><br/>
								<tt>...</tt><br/>
								<tt>task 5 completed sucessfully</tt><br/>
								<tt>task 6 completed with error [java.lang.InterruptedException: sleep interrupted]</tt>
							</div>
						</li>
						<li>at this point, you will notice that the application has exited, without displaying the job execution results</li>
						<li>restart the application: you should now see a message like:<br/>
							<div align="left" style="background-color: #D0D0D0"/>
								<tt>found jobs in persistence store: [the_job_uuid]</tt><br/>
								<tt>loaded job 'the_job_uuid' from persistence store ...</tt><br/>
							</div>
						</li>
