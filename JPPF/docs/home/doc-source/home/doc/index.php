<?php $currentPage="Documentation" ?>
$template{name="about-page-header" title="Documentation"}$
  <h1 align="center">Documentation links</h1>

	<div align="justify">
    <div class="column_left" style="text-align: justify">
      <a name="guide"></a>
      <div class="blockWithHighlightedTitle">
        $template{name="title-with-icon" img="/images/icons/documentation.png" title="User Guides" heading="h3"}$
        <table style="width: 100%"><tr>
          <td valign="top">
            <ul class="samplesList">
              <li><a href="/doc/6.2">JPPF 6.2</a></li>
              <li><a href="/doc/6.1">JPPF 6.1</a></li>
              <li><a href="/doc/6.0">JPPF 6.0</a></li>
            </ul>
          </td>
          <td valign="top">
            <ul class="samplesList">
              <li><a href="/doc/5.2">JPPF 5.2</a></li>
              <li><a href="/doc/5.1">JPPF 5.1</a></li>
              <li><a href="/doc/4.2">JPPF 4.2</a></li>
            </ul>
          </td>
          <td valign="top">
            <ul class="samplesList">
              <li><a href="/doc/3.3">JPPF 3.3</a></li>
              <li><a href="/doc/2.5">JPPF 2.5</a></li>
            </ul>
          </td>
        </tr></table>
      <br></div><br><br>

      <a name="csdoc"></a>
      <div class="blockWithHighlightedTitle">
        $template{name="title-with-icon" img="/images/icons/csharp2.png" title="C-Sharp docs" heading="h3"}$
        <table style="width: 100%"><tr>
          <td valign="top">
            <ul class="samplesList">
              <li><a href="/csdoc/6.0">JPPF 6.0</a></li>
              <li><a href="/csdoc/5.2">JPPF 5.2</a></li>
              <li><a href="/csdoc/5.1">JPPF 5.1</a></li>
            </ul>
          </td>
          <td valign="top">
            <ul class="samplesList">
              <li><a href="/csdoc/5.0">JPPF 5.0</a></li>
            </ul>
          </td>
        </tr></table>
      <br></div>
    </div>

    <div class="column_right" style="text-align: justify">
      <a name="javadoc"></a>
      <div class="blockWithHighlightedTitle">
        $template{name="title-with-icon" img="/images/icons/javadoc2.png" title="Javadoc" heading="h3"}$
        <table style="width: 100%"><tr>
          <td valign="top">
            <ul class="samplesList">
              <li><a href="/javadoc/6.2">JPPF 6.2</a></li>
              <li><a href="/javadoc/6.1">JPPF 6.1</a></li>
              <li><a href="/javadoc/6.0">JPPF 6.0</a></li>
            </ul>
          </td>
          <td valign="top">
            <ul class="samplesList">
              <li><a href="/javadoc/5.2">JPPF 5.2</a></li>
              <li><a href="/javadoc/5.1">JPPF 5.1</a></li>
              <li><a href="/javadoc/5.0">JPPF 5.0</a></li>
            </ul>
          </td>
          <td valign="top">
            <ul class="samplesList">
              <li><a href="/javadoc/4.2">JPPF 4.2</a></li>
              <li><a href="/javadoc/3.3">JPPF 3.3</a></li>
              <li><a href="/javadoc/2.5">JPPF 2.5</a></li>
            </ul>
          </td>
        </tr></table>
      <br></div><br><br>

      <a name="development"></a>
      <div class="blockWithHighlightedTitle">
        $template{name="title-with-icon" img="/images/icons/development.png" title="Still being developed" heading="h3"}$
        <ul class="samplesList">
          <li><a href="/doc/6.3">JPPF 6.3 User Guide</a></li>
          <li><a href="/javadoc/6.3">JPPF 6.3 Javadoc</a></li>
        </ul>
      <br></div>
    </div>
  </div>
	<br>
$template{name="about-page-footer"}$
