...
set CONFIG_FILE="config\jppf-node.properties"
:: set the default value
set JAVA_PATH=java
:: try and parse from the config file
call:getvalue %CONFIG_FILE% "jppf.java.path" JAVA_PATH

call %JAVA_PATH% -cp config;lib/* -Xmx16m -Dlog4j.configuration=log4j-node.properties -Djppf.config=jppf-node.properties -Djava.util.logging.config.file=config/logging-node.properties org.jppf.node.NodeLauncher
goto:eof
