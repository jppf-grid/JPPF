...
public class TestSimpleEmbeddedGrid extends BaseTest {
  ...
  public static void setup() throws Exception {
    ...
    ConfigurationHelper.setLoggerLevels(CONFIG_ROOT_DIR + "log4j-embedded-grid.properties");
    final Map<String, Object> bindings = new HashMap<>();
    bindings.put("$nbDrivers", nbDrivers);
    bindings.put("$nbNodes", nbNodes);

    bindings.put("$n", 1);
    String path = CONFIG_ROOT_DIR + "driver.template.properties";
    ...
    path = CONFIG_ROOT_DIR + "node.template.properties";
    Assert.assertTrue(new File(path).exists());
