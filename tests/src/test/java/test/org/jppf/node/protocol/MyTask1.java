...
public class MyTask1 extends AbstractTask<String> {
  ...
  public void run() {
    ...
    try {
      ...
      final MyTask2 task2 = new MyTask2(duration, submitAndLeave);
      job.add(task2);
      final JPPFClient client = getClient();
      System.out.println("JPPFClient class loader: " + client.getClass().getClassLoader());
      System.out.println("MyTask2    class loader: " + task2.getClass().getClassLoader());
      if (submitAndLeave) {
        job.getSLA().setCancelUponClientDisconnect(false);
        listener = new AwaitTaskNotificationListener(client.awaitWorkingConnectionPool().awaitWorkingJMXConnection(), MyTask2.START_NOTIF);
      }
