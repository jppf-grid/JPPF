...
public class TestOfflineNode extends AbstractNonStandardSetup {
  ...
  public static void setup() throws Exception {
    print(false, false, "class path:\n%s", SystemUtils.prettySystemClassPath());
    ...
    for (int i=0; i<cp.size(); i++) {
      if ("target/classes".equals(cp.get(i))) {
        index = i;
        break;
      }
    }
    if (index >= 0) cp.remove(index);
    print(false, false, "setup node classpath: %s", cp);
    cp.add("../server/target/classes");
    testConfig.driver.log4j = CONFIG_ROOT_DIR + "offline_node/log4j-driver.properties";
    testConfig.node.log4j = CONFIG_ROOT_DIR + "offline_node/log4j-node.properties";
