 ...
 */
public abstract class AbstractDatabaseSetup extends AbstractNonStandardSetup implements DataSourceConstants {
  /**
   * The database server.
   */
  ...
  protected static TestConfiguration dbSetup(final String prefix, final boolean useDB) throws Exception {
    ConfigurationHelper.setLoggerLevel(Level.DEBUG, "org.jppf.persistence");
    if (useDB) {
      print(false, false, "starting H2 server");
      h2Server = Server.createTcpServer().start();
      print(false, false, "H2 server started, creating table");
      // create the test table
      Class.forName(DataSourceConstants.DB_DRIVER_CLASS);
    ...
    final String dir = (prefix == null) ? CONFIG_ROOT_DIR.substring(0, CONFIG_ROOT_DIR.length() - 1) : CONFIG_ROOT_DIR + prefix;
    ...
    config.node.classpath.addAll(getMatches(JARS, "*junit*"));
    return config;
  }
