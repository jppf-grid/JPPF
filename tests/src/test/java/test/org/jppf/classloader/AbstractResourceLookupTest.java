...
public abstract class AbstractResourceLookupTest extends AbstractNonStandardSetup {
  ...
  public static class MyTask extends AbstractTask<String> {
    ...
    public void run() {
      ...
      driverResource2 = lookup("target/test-classes/test-resources/driver1/driver-resource-1.txt"); // in file system only
      clientResource1 = lookup("client-resource-1.txt"); // in classpath
      clientResource2 = lookup("target/test-classes/test-resources/client1/client-resource-1.txt"); // in file system only
      setResult("success");
    }
