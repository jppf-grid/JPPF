 ...
 */
public class TestJPPFTask extends BaseTest {
  /**
   ...
   */
  @BeforeClass
  public static void setupTestJPPFTask() throws Exception {
    final TestConfiguration config = TestConfiguration.newDefault();
    config.node.classpath.add("target/classes");
   ...
   */
  @AfterClass
  public static void cleanupTestJPPFTask() throws Exception {
    BaseSetup.cleanup();
  }
  ...
  public void testTaskTimeoutStart() throws Exception {
    ...
    StandardTasks.MyTask task = new StandardTasks.MyTask(2*timeout);
    task.setTimeoutSchedule(new JPPFSchedule(timeout));
    job.add(task).setId("task 2");
    final List<Task<?>> results = client.submit(job);
    assertNotNull(results);
    assertEquals(results.size(), nbTasks);
    task = (StandardTasks.MyTask) results.get(1);
  ...
  public void testComputeCallable() throws Exception {
    final int nbTasks = 1;
    final JPPFJob job = BaseTestHelper.createJob(ReflectionUtils.getCurrentMethodName(), false, nbTasks, MyComputeCallableTask.class, StandardTasks.MyComputeCallable.class.getName());
  ...
  public void testComputeCallableThrowingException() throws Exception {
    final int nbTasks = 1;
    final JPPFJob job = BaseTestHelper.createJob(ReflectionUtils.getCurrentMethodName(), false, nbTasks, MyComputeCallableTask.class, StandardTasks.MyExceptionalCallable.class.getName());
    callableResult = "test successful";
    final List<Task<?>> results = client.submit(job);
    assertNotNull(results);
    assertEquals(results.size(), nbTasks);
  ...
  public void testTaskLocalNotifications() throws Exception {
    final int nbTasks = 1;
    final JPPFJob job = BaseTestHelper.createJob(ReflectionUtils.getCurrentMethodName(), false, nbTasks, StandardTasks.NotifyingTask.class);
    int count = 0;
    for (Task<?> task: job) task.setId("NotifyingTask " + ++count);
    final List<Task<?>> results = client.submit(job);
    assertNotNull(results);
    assertEquals(results.size(), nbTasks);
    final StandardTasks.NotifyingTask task = (StandardTasks.NotifyingTask) results.get(0);
  ...
  public void testTaskJMXNotifications() throws Exception {
    ...
    final JPPFJob job = BaseTestHelper.createJob(ReflectionUtils.getCurrentMethodName(), false, nbTasks, StandardTasks.NotifyingTask2.class);
    try {
  ...
  private static void resetNodeCOunter() throws Exception {
    ...
    client.submit(BaseTestHelper.createJob(ReflectionUtils.getCurrentClassName() + ":resetCounter", false, 1, StandardTasks.CounterResetTask.class));
  }
