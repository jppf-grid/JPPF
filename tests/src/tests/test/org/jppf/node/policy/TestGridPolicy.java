...
public class TestGridPolicy extends Setup1D2N1C {
  ...
  public void testWorkingFalseJavaPolicy() throws Exception {
    ...
    NodeSelector masterSelector = new ExecutionPolicySelector(new Equal(JPPFProperties.PROVISIONING_MASTER.getName(), true));
    try {
      Thread.sleep(1000L);
      ...
      jmx.getNodeForwarder().provisionSlaveNodes(masterSelector, 0);
      while (jmx.nbNodes() > 2) Thread.sleep(10L);
    }
  }
}
