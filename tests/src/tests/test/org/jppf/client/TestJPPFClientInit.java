...
public class TestJPPFClientInit extends Setup1D1N {
   ...
   */
  private static Logger log = LoggerFactory.getLogger(TestJPPFClientInit.class);
  ...
  public void testSystemProperties() throws Exception {
    JPPFJob job = BaseTestHelper.createJob(ReflectionUtils.getCurrentClassAndMethod(), false, false, 1, LifeCycleTask.class, 0L);
    job.getJobTasks().get(0).setId("task0");
    UncaughtExceptionHandler oldEh = Thread.getDefaultUncaughtExceptionHandler();
    MyUncaughtExceptionHandler myEh = new MyUncaughtExceptionHandler();
    ...
    try (JPPFClient client = new JPPFClient()) {
      ...
      List<Task<?>> results = job.get(3L, TimeUnit.SECONDS);
