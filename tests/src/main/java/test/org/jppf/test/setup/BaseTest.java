...
public class BaseTest {
  ...
  /**
   * Root directory for the test config files.
   */
  public static final String CONFIG_ROOT_DIR = "target/test-classes/config/";
  /**
  ...
  public static class BaseTestClassWatcher extends TestWatcher {
    ...
    protected void starting(final Description description) {
      ...
      final File slavesDir = new File(dir, "target/slave_nodes");
      if (slavesDir.exists() && !deletePath(slavesDir)) print(true, true, "Could not delete '%s'", slavesDir);
      org.apache.log4j.PropertyConfigurator.configure(CONFIG_ROOT_DIR + "log4j-client.properties");
      // redirect System.out and System.err to files
      stdOut = System.out;
      stdErr = System.err;
      try {
        System.setOut(new TestPrintStream("target/std_out.log"));
        System.setErr(new TestPrintStream("target/std_err.log"));
          ...
          for (final File subdir: subdirs) {
            ...
            if (logFiles != null) {
              for (final File logFile: logFiles) {
                ...
                try {
                  StreamUtils.copyFile(logFile, new File(targetDir, path));
