  ...
  public static JPPFClient setup(final int nbDrivers, final int nbNodes, final boolean createClient, final boolean checkDriversAndNodes,
    ...
    if (testWithEmbeddedGrid) {
      ...
      for (int i=0; i<nbDrivers; i++) {
        ...
        BaseTest.print(false, false, "starting %s", drivers[i].getName());
      ...
      for (int i=0; i<nbNodes; i++) {
        ...
        BaseTest.print(false, false, "starting %s", nodes[i].getName());
        ThreadUtils.startDaemonThread(nodes[i], nodes[i].getName().trim() + "-Launcher");
      }
    }
    if (createClient) {
      final String uuid = "client-" + clientUuidSequence.incrementAndGet();
      BaseTest.print(false, false, "starting JPPF client with uuid = %s", uuid);
      client = createClient(uuid, true, config, listeners);
      BaseTest.print(false, false, "started client with uuid = %s", uuid);
      if (checkDriversAndNodes) checkDriverAndNodesInitialized(client, nbDrivers, nbNodes, false);
      BaseTest.print(false, false, "checked grid connection state");
    } else {
      JPPFConfiguration.reset();
    }
    return client;
