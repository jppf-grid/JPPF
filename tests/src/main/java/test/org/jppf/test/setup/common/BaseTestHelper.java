...
public class BaseTestHelper {
  ...
  public static void generateClientThreadDump() throws Exception {
    try (final Diagnostics diag = new Diagnostics("client")) {
      ...
      FileUtils.writeTextFile("target/client_thread_dump.log", text);
    }
  }
    ...
    for (final JMXDriverConnectionWrapper jmx: jmxConnections) {
      if ((jmx != null) && jmx.isConnected()) {
        try {
          ...
          FileUtils.writeTextFile("target/driver_thread_dump_" + jmx.getPort() + ".log", text);
        } catch (final Exception e) {
        ...
        try {
          ...
          FileUtils.writeTextFile("target/server_debug_" + jmx.getPort() + ".log", dump);
        } catch (@SuppressWarnings("unused") final Exception e) {
          log.error("failed to get debug dump for {}", jmx, e);
        }
        try {
          ...
          for (final Map.Entry<String, InvocationResult<ThreadDump>> entry: dumpsMap.entrySet()) {
            ...
            if (entry.getValue().isException()) {
              ...
              FileUtils.writeTextFile("target/node_thread_dump_" + (info == null ? uuid : info.getPort()) + ".log", text);
            }
          }
        } catch (final Exception e) {
