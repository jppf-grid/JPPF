...
public class Setup1D2N1C
{
   ...
   */
  protected static NodeProcessLauncher[] node = new NodeProcessLauncher[2];
  /**
   * The node to lunch for the test.
   */
  ...
  protected static void stopProcesses()
  {
    try
    {
      if (node != null)  for (NodeProcessLauncher n: node) n.stopProcess();
  ...
  public static void setup() throws Exception
  {
    ...
    if (launchProcesses)
    {
      ...
      for (int i=0; i<node.length; i++)
      {
        ...
        Thread.sleep(51L);
        node[i] = new NodeProcessLauncher(i+1);
        node[i].startProcess();
      }
    }
