...
package org.jppf.example.provisioning.client;

import java.util.*;

import org.jppf.client.*;
import org.jppf.example.provisioning.master.NodeProvisioningMBean;
...
public class Runner {
  ...
  public static void main(final String[] args) {
    ...
    try {
      ...
      ExecutionPolicy masterPolicy = new Equal("jppf.node.master", true);
      NodeSelector masterSelector = new NodeSelector.ExecutionPolicySelector(masterPolicy);
      // specifiy configuration overrides for the new slave nodes
      ...
      String[] signature = {Integer.class.getName(), TypedProperties.class.getName()};
      // actually request that 4 slave nodes be provisioned
      forwarder.forwardInvoke(masterSelector, NodeProvisioningMBean.MBEAN_NAME, "provisionSlaveNodes", new Object[] { 4, overrides }, signature);
      Thread.sleep(5000L);
      // now we check that we have effectively 4 slave nodes for each master
      Map<String, Object> resultsMap = forwarder.forwardGetAttribute(masterSelector, NodeProvisioningMBean.MBEAN_NAME, "NbSlaves");
      ...
      forwarder.forwardInvoke(masterSelector, NodeProvisioningMBean.MBEAN_NAME, "provisionSlaveNodes", new Object[] { 0, null }, signature);
      Thread.sleep(5000L);
      // and we check that each master now has zero slave nodes
      resultsMap = forwarder.forwardGetAttribute(masterSelector, NodeProvisioningMBean.MBEAN_NAME, "NbSlaves");
      for (Map.Entry<String, Object> entry: resultsMap.entrySet()) {
        if (entry.getValue() instanceof Throwable) System.out.println("node " + entry.getKey() + " raised " + ExceptionUtils.getStackTrace((Throwable) entry.getValue()));
