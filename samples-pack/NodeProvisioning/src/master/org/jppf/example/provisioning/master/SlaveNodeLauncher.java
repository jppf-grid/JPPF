...
public class SlaveNodeLauncher implements Runnable {
  ...
  public SlaveNodeLauncher(final String name, final List<String> classpath) {
    ...
    slaveDir = new File(System.getProperty("user.dir"), name);
  ...
  private Process startProcess() throws Exception {
    ...
    command.add("-Djppf.config=jppf-node.properties");
    command.add("-Dlog4j.configuration=log4j-node.properties");
    for (String opt: jvmOptions) command.add(opt);
    command.add(MAIN_CLASS);
    command.add("" + processPort);
    if (log.isDebugEnabled()) log.debug("process command:\n" + command);
  ...
  private void fireProcessStarted() {
    if (log.isDebugEnabled()) log.debug("process [" + process + "] has started");
  ...
  private void fireProcessStopped() {
    if (log.isDebugEnabled()) log.debug("process [" + process + "] has stopped");
    SlaveNodeLauncherEvent event = new SlaveNodeLauncherEvent(this);
