 ...
 */
public class ReconnectNodeAction extends AbstractTopologyAction {
  /**
   * Logger for this class.
   */
  private static Logger log = LoggerFactory.getLogger(ReconnectNodeAction.class);
  /**
   ...
   */
  public ReconnectNodeAction(final boolean interruptIfRunning, final String name) {
      ...
      public void run() {
        ...
        for (final Map.Entry<TopologyDriver, Collection<String>> entry: map.entrySet()) {
          try {
            ...
            if (debugEnabled) log.debug("invoking reconnect with interrupt={} for the nodes: {}", interruptIfRunning, entry.getValue());
