...
public class ScriptedMouseListener implements MouseListener {
  ...
  private void invokeScript(final MouseEvent event, final String eventType) {
    ...
    try {
      ...
      if (debugEnabled) log.debug(String.format("executed % script in %d ms for %s", language, elapsed, option));
    } catch (JPPFScriptingException e) {
      log.error(String.format("Error while executing %s script for %s%nScript =%n%s%nException = %s" , language, option, scriptText, e));
    } finally {
