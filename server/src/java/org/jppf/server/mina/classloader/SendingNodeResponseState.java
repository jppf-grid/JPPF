...
class SendingNodeResponseState extends ClassServerState
{
	 ...
	 */
	public SendingNodeResponseState(MinaClassServer server)
	{
		super(server);
	}
	...
	public void endTransition(IoSession session) throws Exception
	{
		...
		if (debugEnabled) log.debug("node: session " + session.getId() + ", response [" + context.getResource().getName() + "] sent to the node");
