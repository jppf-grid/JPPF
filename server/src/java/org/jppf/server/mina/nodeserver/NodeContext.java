 ...
 */
public class NodeContext extends MinaContext<NodeState>
{
	/**
	 ...
	 */
	public void handleException(IoSession session)
	{
		if (getBundler() != null) getBundler().dispose();
		JPPFDriver driver = JPPFDriver.getInstance();
		driver.getMinaNodeServer().closeNode(session, this);
		if ((bundle != null) && !JPPFTaskBundle.State.INITIAL_BUNDLE.equals(bundle.getBundle().getState()))
		{
			driver.getJobManager().jobReturned(bundle, new IoSessionWrapper(session));
