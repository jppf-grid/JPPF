...
public class JPPFNode implements MonitoredNode {
	...
	/**
	 * Determines whether the debug level is enabled in the log4j configuration,
	...
	public void run() {
		...
		if (notifying)
			fireNodeEvent(NodeEvent.DISCONNECTED);
	}

	/**
	 * Perform the main execution loop for this node. At each iteration, this
	 * method listens for a task to execute, receives it, executes it and sends
	 * the results back.
	 * 
	 * @throws Exception
	 *             if an error was raised from the underlying socket connection
	 *             or the class loader.
	 */
	public void perform() throws Exception {
		while (!stopped) {
			...
			if (buildNumber < p) {
				JPPFNodeReloadNotification notif = new JPPFNodeReloadNotification(
						"detected new build number: " + p
	...
	public synchronized void init() throws Exception {
		if (socketClient == null)
			initSocketClient();
		if (notifying)
			fireNodeEvent(NodeEvent.START_CONNECT);
		System.out
				.println("JPPFNode.init(): Attempting connection to the JPPF driver");
		socketInitializer.initializeSocket(socketClient);
		if (notifying)
			fireNodeEvent(NodeEvent.END_CONNECT);
		System.out.println("JPPFNode.init(): Reconnected to the JPPF driver");

		TypedProperties props = JPPFConfiguration.getProperties();

		threadPool = Executors.newFixedThreadPool(props.getInt(
	...
	/**
	 * Read a task from the socket connection, along with its header
	 * information.
	 * 
	 * @return a pair of <code>JPPFTaskBundle</code> and a <code>List</code>
	 *         of <code>JPPFTask</code> instances.
	 * @throws Exception
	 *             if a error is raised while reading the task data.
	 */
	...
	/**
	 * Deseralize the objects read from the socket, and reload the appropriate
	 * classes if any class change is detected.<br>
	 * A class change is triggered when an <code>InvalidClassException</code>
	 * is caught. Upon catching this exception, the class loader is
	 * reinitialized and the class are reloaded.
	 * 
	 * @param bytes
	 *            the array of bytes from which the objects are deserialized.
	 * @return an array of 3 objects desrialied from the byte array.
	 * @throws Exception
	 *             if the classes could not be reloaded or an error occurred
	...
	/**
	 * Perform the deserialization of the objects received through the socket
	 * connection.
	 * 
	 * @param bytes
	 *            the buffer containing the serialized objects.
	...
	private Object[] deserializeObjects(byte[] bytes) throws Exception {
		...
		JPPFTaskBundle bundle = (JPPFTaskBundle) helper.readNextObject(dis,
				false);
	 ...
	 * 
	 * @param bundle
	 *            the task wrapper to send along.
	 * @param tasks
	 *            the list of tasks with their result field updated.
	 * @throws Exception
	 *             if an error occurs while writitng to the socket stream.
	 */
	private void writeResults(JPPFTaskBundle bundle, List<JPPFTask> tasks)
			throws Exception {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		DataOutputStream dos = new DataOutputStream(baos);
		long elapsed = System.currentTimeMillis()
				- bundle.getNodeExecutionTime();
	 ...
	 * 
	 * @return a <code>ClassLoader</code> used for loading the classes of the
	 *         framework.
	 */
	 ...
	 * 
	 * @throws Exception
	 *             if an error occcurs while instantiating the class loader.
	 */
	private void initHelper() throws Exception {
		Class c = getClassLoader().loadJPPFClass(
				"org.jppf.utils.ObjectSerializerImpl");
		Object o = c.newInstance();
		serializer = (ObjectSerializer) o;
		c = getClassLoader().loadJPPFClass(
	...
	/**
	 * Get a reference to the JPPF container associated with an application
	 * uuid.
	 * 
	 * @param uuid
	 *            the uuuid to find the container for.
	 * @return a <code>JPPFContainer</code> instance.
	 * @throws Exception
	 *             if an error occcurs while getting the container.
	 */
