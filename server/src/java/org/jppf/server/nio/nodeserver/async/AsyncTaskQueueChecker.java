 ...
 */
public class AsyncTaskQueueChecker extends AbstractAsyncTaskQueueChecker {
  /**
   * Logger for this class.
   */
  private static final Logger log = LoggerFactory.getLogger(AsyncTaskQueueChecker.class);
  /**
   ...
   */
  AsyncTaskQueueChecker(final AsyncNodeNioServer server, final JPPFPriorityQueue queue, final JPPFStatistics stats, final JPPFBundlerFactory bundlerFactory) {
  ...
  private AbstractBaseNodeContext<?> findIdleChannelIndex(final ServerJob job) {
    ...
    while (iterator.hasNext()) {
      ...
      synchronized(channel.getMonitor()) {
        ...
        if (!checkExecutionPolicy(channel, job, policy, info, nbJobChannels)) continue;
        if (!checkMaxNodeGroups(channel, job)) continue;
        if (!checkDesiredConfiguration(desiredConfiguration, channel, job, readyNodes, nbReservedNodes)) continue;
    ...
    if (!toRemove.isEmpty()) {
      for (final AbstractBaseNodeContext<?> c: toRemove) removeIdleChannelAsync(c);
    }
   ...
   */
  private boolean checkDesiredConfiguration(final TypedProperties config, final AbstractBaseNodeContext<?> channel, final ServerJob job, final Collection<String> readyNodes, final int nbReservedNodes) {
    if (config != null) {
  ...
  */
   ...
   */
  private boolean checkExecutionPolicy(final AbstractBaseNodeContext<?> channel, final ServerJob job, final ExecutionPolicy policy, final JPPFSystemInformation info, final int nbJobChannels) {
