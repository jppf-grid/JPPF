...
class PeerNodeResultSender implements TaskCompletionListener
{
  ...
  public void sendResults(final ServerJob bundleWrapper) throws Exception
  {
    try
    {
      ...
      if (debugEnabled) log.debug("Sending result bundle " + bundle);
      ...
      socketClient.flush();
      if (debugEnabled) log.debug("sent result bundle");
    }
    finally
    {
      result = null;
  ...
  public synchronized void taskCompleted(final ServerJob nodeResult)
  {
    ...
    if (debugEnabled)
    {
      log.debug("Received results for : " + resultJob + ", size=" + nodeResult.getTasks().size() + ", pending tasks=" + pendingTasksCount);
    }
