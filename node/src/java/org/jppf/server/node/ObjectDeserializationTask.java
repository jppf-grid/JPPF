...
public class ObjectDeserializationTask implements Callable<ObjectDeserializationTask> {
   ...
   */
  private static final Logger log = LoggerFactory.getLogger(JPPFContainer.class);
  /**
   ...
   */
  public ObjectDeserializationTask(final JPPFContainer cont, final DataLocation dl, final int index) {
    this.cont = cont;
    this.dl = dl;
    this.index = index;
  }
  ...
  public ObjectDeserializationTask call() {
    ...
    try {
      ...
      if (traceEnabled) log.trace("deserializing object index = " + index);
