...
rem
rem Options
rem install                Install the service using JPFPNode as service name.
...
"%EXECUTABLE%" //IS//%SERVICE_NAME% ^
    ...
    --StartPath "%JPPF_HOME%" ^
    --StopPath "%JPPF_HOME%" ^
    --StartClass org.jppf.node.NodeLauncher ^
    --StopClass java.lang.System ^
    ++JvmOptions "-Dlog4j.configuration=log4j-node.properties" ^
    ++JvmOptions "-Djppf.config=jppf-node.properties" ^
    ++JvmOptions "-Djava.util.logging.config.file=config/logging-node.properties" ^
    --JvmMs 16 ^
    --JvmMx 32 ^
    --Startup auto
