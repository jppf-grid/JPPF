...
public class ResourceCache
{
	 ...
	 * Get a location for the resource with the specified name.
	 * @param name the name of the resource to lookup.
	 * @return a URL, or null if the resource is not found in the cache.
	...
	public URL getResourceURL(String name) throws MalformedURLException
	{
		List<String> locations = cache.get(name);
		if ((locations == null) || locations.isEmpty()) return null;
		File file = new File(locations.get(0));
	 ...
	 * @param definitions an array of byte array definitions.
	 * @throws Exception if any I/O error occurs.
	 */
	public void registerResources(String name, byte[]...definitions) throws Exception
	{
