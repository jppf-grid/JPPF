...
public class Matrix implements Serializable {
   ...
   * @return this matrix.
   */
  public Matrix assignRandomValues() {
    Random rand = new Random(System.nanoTime());
    for (int i = 0; i < values.length; i++) {
      // values in ]-RANDOM_RANGE, +RANDOM_RANGE[
      for (int j = 0; j < values[i].length; j++) values[i][j] = RANDOM_RANGE * (2d * rand.nextDouble() - 1d);
    }
    return this;
  }
  ...
  public double[] multiplyRow(final int n, final Matrix matrix) {
    ...
    for (int col = 0; col < size; col++) {
      double sum = 0d;
      for (int row = 0; row < size; row++) sum += matrix.getValueAt(row, col) * getValueAt(n, row);
      result[col] = sum;
    }
    return result;
  }
