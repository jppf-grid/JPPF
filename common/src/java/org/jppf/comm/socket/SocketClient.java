...
public class SocketClient extends AbstractSocketWrapper
{
     ...
     * 
     * @param host
     *            the remote host this socket client connects to.
     * @param port
     *            the remote port on the host this socket client connects to.
     * @throws ConnectException
     *             if the connection fails.
     * @throws IOException
     *             if there is an issue with the socket streams.
     */
    public SocketClient(String host, int port) throws ConnectException,
            IOException
    {
     ...
     *            the remote port on the host this socket client connects to.
     * @param serializer
     *            the object serializer used by this socket client.
     * @throws ConnectException
     *             if the connection fails.
     * @throws IOException
     *             if there is an issue with the socket streams.
     ...
     * 
     * @param socket
     *            the underlying socket this socket client wraps around.
     ...
     *            the object to send.
     * @throws IOException
     *             if the underlying output stream throws an exception.
     ...
     * 
     * @param timeout
     *            timeout after which the operation is aborted. A timeout of zero is interpreted as
     *            an infinite timeout.
     * @return the object that was read from the underlying input stream or null if the operation
     *         timed out.
     * @throws ClassNotFoundException
     *             if the socket connection is closed.
     * @throws IOException
     *             if the underlying input stream throws an exception.
     * @see org.jppf.comm.socket.SocketWrapper#receive(int)
     */
    public Object receive(int timeout) throws ClassNotFoundException,
            IOException
    {
        checkOpened();
        Object o = null;
    ...
    public ObjectSerializer getSerializer()
    {
        if (serializer == null)
        {
            ...
            try
            {
                serializer = (ObjectSerializer) Class.forName(name)
                        .newInstance();
     ...
     * 
     * @param serializer
     *            an <code>ObjectSerializer</code> instance.
    ...
    {
        return "SocketClient (" + this.host + ":" + this.port + ") open:"
                + this.opened;
    }
}
