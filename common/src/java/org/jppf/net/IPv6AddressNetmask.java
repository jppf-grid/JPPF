...
public class IPv6AddressNetmask extends IPv6AddressPattern {
  ...
  private static String netmaskToRange(final String source) {
    ...
    // ":::" or more than one "::"
    final int parts = RegexUtils.COLUMN_PATTERN.split(sourceIP).length;
    ...
    if (sourceIP.contains("::")) {
      final StringBuilder zeroes = new StringBuilder(":0");
    ...
    final String[] ip = RegexUtils.COLUMN_PATTERN.split(sourceIP);
    ...
    for (int i = 0; i < 8; i++) {
      final int maskBits = 16 * (i + 1) - netmask;
      ...
      if (maskBits >= 16) {
        ...
        final int b = Integer.parseInt(ip[i], 16);
  ...
  public static void main(final String[] args) {
    ...
    final String[] ipv6patterns = { "1080:0:0:0:8:800:200C:417A", ":0::::::",
    ...
    for (int i = 0; i < ipv6patterns.length; i++) {
      try {
        final IPv6AddressNetmask p = new IPv6AddressNetmask(ipv6patterns[i]);
