 ...
 * <p>Each profile is created by an instance of {@link ProfileServerProvider}. This instance is found as follows. Suppose the given <code><em>profile</em></code> looks like <code>TLS</code>. Then the
 * factory will attempt to find the appropriate {@link ProfileServerProvider} for <code><em>tls</em></code>. Suppose the given <code><em>profile</em></code> looks like <code>SASL/PLAIN</code>. Then
 * the factory will attempt to find the appropriate {@link ProfileServerProvider} for <code><em>sasl</em></code>. The <code><em>profile</em></code> string passed in to the factory is converted into
 ...
 * <blockquote> <code><em>pkg</em>.<em>profile</em>.ServerProvider</code> </blockquote>
 ...
 * <p>If the <code>jmx.remote.profile.provider.class.loader</code> key is not present in the <code>environment</code> parameter, the class loader that loaded the <code>ProfileServerFactory</code> class
 * is used.
 ...
 * <p>Otherwise, a problem with the found provider is signalled by a {@link ProfileProviderException} whose {@link ProfileProviderException#getCause() <em>cause</em>} indicates the underlying exception,
 * as follows:
 * <ul>
 ...
 * <p>Once a provider is found, the result of the <code>createProfile</code> method is the result of calling {@link ProfileServerProvider#createProfile(String,Map) createProfile} on the provider.
 * <p>The <code>Map</code> parameter passed to the <code>ProfileServerProvider</code> is a new read-only copy of the <code>environment</code> parameter to {@link #createProfile(String, Map)
 * ProfileServerFactory.createProfile}, or an empty <code>Map</code> if that parameter is null. If the <code>jmx.remote.profile.provider.class.loader</code> key is not present in the
 * <code>environment</code> parameter, it is added to the new read-only <code>Map</code>. The associated value is the class loader that loaded the <code>ProfileServerFactory</code> class.
 */
public final class ProfileServerFactory {
   ...
   * @param environment a read-only Map containing named attributes to determine how the profile is created.
   * Keys in this map must be Strings. The appropriate type of each associated value depends on the attribute.</p>
   * @return a <code>ProfileServer</code> representing the new profile. Each successful call to this method produces a different object.
   * @exception NullPointerException if <code>profile</code> is null.
   * @throws ProfileProviderException .
   */
  public static ProfileServer createProfile(final String profile, final Map<String, ?> environment) throws ProfileProviderException {
    final String pkgs = resolvePkgs(environment);
    final ClassLoader loader = resolveClassLoader(environment);
    Map<String, Object> env = null;
    if (environment == null) env = new HashMap<>();
    else env = new HashMap<>(environment);
    env.put(PROFILE_PROVIDER_CLASS_LOADER, loader);
    env = Collections.unmodifiableMap(environment);
   ...
   * @return .
   */
  private static String resolvePkgs(final Map<String, ?> env) {
    String pkgs = null;
   ...
   * @throws ProfileProviderException .
   */
  private static ProfileServerProvider getProvider(final String profile, final String pkgs, final ClassLoader loader) throws ProfileProviderException {
    Class<?> providerClass = null;
    final ProfileServerProvider provider = null;
    Object obj = null;
    ...
    while (tokenizer.hasMoreTokens()) {
      ...
      final String className = (pkg + "." + p + ".ServerProvider");
      try {
      ...
      if (!(obj instanceof ProfileServerProvider)) {
        final String msg = "Provider not an instance of " + ProfileServerProvider.class.getName() + ": " + obj.getClass().getName();
   ...
   * @return .
   */
  private static ClassLoader resolveClassLoader(final Map<String, ?> environment) {
    ...
    if (loader == null) loader = ProfileServerFactory.class.getClassLoader();
    return loader;
  }
}
